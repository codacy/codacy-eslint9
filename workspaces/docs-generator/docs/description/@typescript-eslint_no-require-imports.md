...
If you prefer to instead use `require()` syntax, we recommend you use this rule with [`allowAsImport`](#allowAsImport) enabled.
...
We don't directly provide a way to _prohibit_ ES Module syntax from being used; consider instead using TypeScript's [`verbatimModuleSyntax`](https://www.typescriptlang.org/tsconfig/#verbatimModuleSyntax) option if you find yourself in a situation where you would want this.
