...
export const engineImpl: Engine = async function (
  ...
  // Cast options to any while @types/eslint is not updated
  const eslint = new ESLint(options as any);
...
async function lintFilesInChunks(
  eslint: ESLint,
  files: string[]
...
async function lintFilesChunkByChunk(
  eslint: ESLint,
  chunksOfFiles: string[][]
): Promise<ESLint.LintResult[]> {
...
async function debugAndCountLintIssues(
  ...
  debug(
    `engine: ${lintResults.length} files linted and ${nIssues} issues found`
  );
}
...
function chunkFilesByTotalSize(
  files: string[],
  maxChunkSize: number
): string[][] {
  const chunks: string[][] = [];
  ...
  for (const file of files) {
    try {
      ...
      if (
        ...
        currentChunkSize + size <= maxChunkSize
      ) {
      ...
      console.error(
        `engine: error while getting file size for "${file}": ${error}`
      );
    }
  }
